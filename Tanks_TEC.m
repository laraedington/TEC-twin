% This code produces the TEC-twin response of the Cascading Tanks nonlinear
% benchmark system.
% Data is available at https://doi.org/10.4121/12960104.v1.
%
...
% Load system input/response data (available at https://doi.org/10.4121/12960104.v1)
load dataBenchmark.mat
F=uEst;
X_D=yEst;

% Sampling parameters and initial conditions
L=length(F);
T=Ts;
time=Ts*(0:1:length(F)-1);
y_initial=[0;X_D(1)];
...
prior_min=[0.055 0.05 0.055 0.05 0];    % mean of normal prior
prior_max=[0.01 0.01 0.01 0.01 0.005];  % variance of normal prior
...
for i=1:translate:L-sec_length+1
    ...
    while count<N
        ...
        for j=1:num_pars
                theta(j)=normrnd(prior_min(model,j),(prior_max(model,j)));
        end
        
        % simulate sampled model
        try
            [t,y]=ode45(@(t,y) tanks(theta(1),theta(2),theta(3),theta(4),theta(5),time_trval,F_tr,t,y),time_trval,y_initial);
            y_sim=real(y(round(i/2):round(sec_end/2),2));
            MSE=sum((data-y_sim).^2)/length(y_sim);
            if MSE<epsilon
    ...
    % Training
    [t,y]=ode45(@(t,y) tanks(PBM(1),PBM(2),PBM(3),PBM(4),PBM(5),time_trval,F_tr,t,y),time_trval,y_initial);
    x_p_tr=y(round(i/2):round(sec_end/2),2);
    
    % Validation
    [t,y]=ode45(@(t,y) tanks(PBM(1),PBM(2),PBM(3),PBM(4),PBM(5),time_trval,F_val,t,y),time_trval,y_initial);
    x_p_val=y(round(i/2):round(sec_end/2),2);
    ...
    while NMSE_DBM_val>1    % NMSE limit
        ...
        % Comment out if using OSA predicitons:
        xd_tr=Yhat_tr;
        xd_val=Yhat_val;
...
% PLOT RESULTS
%=========================================================================
% For plotting start and end of sections:
section_y=[-1000 1000];
...
% ylabel('Water Level','fontsize',30)
% a = get(gca,'XTickLabel');
% set(gca,'XTickLabel',a,'fontsize',30)
% ylim([1 14])
% xlim([0 time(end)])
...
% Plot overflow parameter:
h=figure;
plot_k5=zeros(1,L);
for i=1:translate:L-sec_length+1
    plot_k5(:,i:i+translate-1)=[PBMparameters(5,(i+translate-1)/translate)*ones(1,translate)];
end
plot(time,plot_k5,'LineWidth',4.5)
hold on
for j=1:size(section_x,1)
    plot(section_x(j,:),[-5 500],'k','LineWidth',4.5)
end
hold off
title('Overflow Parameter vs. Time','fontsize',30)
xlabel('Time (s)','fontsize',30)
ylabel('Overflow Parameter','fontsize',30)
a = get(gca,'XTickLabel');
set(gca,'XTickLabel',a,'fontsize',30);
xlim([0 time(end)])
ylim([-0.0005 0.0018])
