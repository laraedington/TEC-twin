...
Fs=1000*omega/2/pi;   % Sampling frequency, (samples/cycle*cycles/second = samples/sec)            
...
prior_min=[10 15 10 150];       % mean (min) of normal (uniform) prior
prior_max=[1  1  1 500];        % var (max) of normal (uniform) prior
...
PBMparameters=zeros(4,length(1:translate:L-sec_length+1));
...
for i=1:translate:L-sec_length+1
    ...
    while count<N    
        ...
        for j=1:num_pars
            if j==num_pars
                theta(j)=prior_min(model,j)+(prior_max(model,j)-prior_min(model,j)).*rand(1);
            else
        ...
        try
            [t,y]=ode45(@(t,y) nonlinear(theta(1),theta(2),0,0,theta(3),0,theta(4),time_trval(1:sec_end/2),F_tr(1:sec_end/2),t,y),time_trval(1:sec_end/2),y_initial);
    ...
    % Training
    [t,y]=ode45(@(t,y) nonlinear(PBM(1),PBM(2),0,0,PBM(3),0,PBM(4),time_trval,F_tr,t,y),time_trval,y_initial);
    x_p_tr=y(round(i/2):round(sec_end/2),1);
    
    % Validation
    [t,y]=ode45(@(t,y) nonlinear(PBM(1),PBM(2),0,0,PBM(3),0,PBM(4),time_trval,F_val,t,y),time_trval,y_initial);
    ...
    % Increase lags by 1 for new model
...
% title('Training: Physics-to-Data Model Fraction vs. Time','fontsize',30)
% xlabel('Time (s)','fontsize',30)
...
plot_k3=zeros(1,L);
for i=1:translate:L-sec_length+1
    plot_k3(:,i:i+translate-1)=[PBMparameters(4,(i+translate-1)/translate)*ones(1,translate)];
end
